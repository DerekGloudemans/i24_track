...
class MCLoader():
    ...
    def __init__(self,directory,mapping_file,ctx,resize = (1920,1080)):
        ...
        # TODO - the loaded config file should specify which cameras are included, and what the regex naming convention is
    ...
    def get_frames(self,target_time, tolerance = 1/60):
        
        # TODO - reimplement this such that each camera gets advanced until it is
...
class GPUBackendFrameGetter:
    def __init__(self,file,device,ctx,buffer_size = 5,resize = (1920,1080)):
        ...
        # TODO - instead of a single file, pass a directory, and a start time
...
def load_queue_vpf(q,file,device,buffer_size,resize):
    ...
    # TODO - instead of a single file, recieves a directory, a camera name, and a first time
    ...
    # instead of looping forever, catch last frame or no return load Decoder for next file, and continue
    ...
    while True:
        if q.qsize() < buffer_size:
            ...
            # TODO - took this from NvDecod pytorch example, this should trigger queue of next video file
            if raw_surface.Empty():
                break
