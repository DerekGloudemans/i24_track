...
class EmbeddingModel(nn.Module):
    def __init__(self, num_features_in, num_anchors=9, feature_size=256,n_outputs = 32):
        ...
        self.conv3 = nn.Conv2d(feature_size, num_anchors*feature_size, kernel_size=1, padding=0)
        self.act3 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        # out is B x C x W x H, with C = feature_size*num_anchors
        out = out.permute(0, 2, 3, 1)
        # now out is of shape B,W,H,C

        # reshape to batch x w x h x n_anchors, feature_size

        # do some fully connected nonsense
        temp = out.reshape(-1,self.feature_size)
        temp2 = self.fc(temp)
        temp2 = self.fc_act(temp2)
        out =  temp2.view(out.shape[0],out.shape[1],out.shape[2],-1)
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, block, layers):
        ...
        self.embeddingModel      = EmbeddingModel(256)
        ...
        self.classificationModel.output.weight.data.uniform_(0-math.log((1.0 - prior) / prior),math.log((1.0 - prior) / prior))
        self.classificationModel.output.bias.data.uniform_(0-math.log((1.0 - prior) / prior),math.log((1.0 - prior) / prior))
        
        #self.embeddingModel.output.weight.data.uniform_(-1/100,1/100)
        #self.embeddingModel.output.bias.data.uniform_(-1/100,1/100)
        

        self.regressionModel.output.weight.data.uniform_(-1/100,1/100)
        self.regressionModel.output.bias.data.uniform_(-1/100,1/100)
    ...
    def forward(self, inputs, LOCALIZE = False,MULTI_FRAME = False):
        ...
        embedding = torch.cat([self.embeddingModel(feature) for feature in features], dim=1)
        
        
        anchors = self.anchors(img_batch)

        if self.training:
            return self.focalLoss(classification, regression, anchors, annotations, embedding)
            ...
            embedding = embedding.reshape(1,-1,embedding.shape[2]).squeeze()
            ...
            embedding = embedding[scores_over_thresh]
            ...
            embedding = embedding[anchors_nms_idx]
            imIndexes = imIndexes[anchors_nms_idx]
            
            return scores,classes,anchorBoxes,imIndexes, embedding
            ...
            finalEmbeddings = torch.Tensor([]).float()
            
            if LOCALIZE:
                return transformed_anchors, classification, embedding
            
            if torch.cuda.is_available():
                finalScores = finalScores.cuda()
                finalAnchorBoxesIndexes = finalAnchorBoxesIndexes.cuda()
                finalAnchorBoxesCoordinates = finalAnchorBoxesCoordinates.cuda()
                finalEmbeddings = finalEmbeddings.cuda()
            ...
            for i in range(classification.shape[2]):
                ...
                classEmbeddings = embedding.squeeze()[scores_over_thresh]
                ...
                finalEmbeddings = torch.cat((finalEmbeddings,classEmbeddings[anchors_nms_idx]))
            ...
            return [finalScores, finalAnchorBoxesIndexes, finalAnchorBoxesCoordinates, finalEmbeddings]
